...
if($_GET['action']=='delete'){
    ...
    //删除单条信息
    _query("DELETE FROM tg_friend WHERE tg_id='{$_GET['id']}' LIMIT 1");
...
//从收信页面跳转
if(isset($_GET['id'])){
    $_row=_fetch_array("SELECT tg_fromuser,tg_content,tg_date,tg_state FROM tg_friend WHERE tg_id='{$_GET['id']}' LIMIT 1");
    if(!$_row){
        _alert_back('该信息不存在');
    }
...
<head>
    ...
    <script type="text/javascript" src="javascript/memeber_friend_detail.js"></script>
</head>
<body>
<?php
//调用header
...
<div id="member">
    ...
    <div id="member_main">
        <h2>好友申请</h2>
        <dl>
            <dd>申 请 人：<?php echo $_html['tg_fromuser']?></dd>
            <dd>申请内容：<strong><?php echo $_html['tg_content']?></strong></dd>
            <dd>发送时间：<?php echo $_html['tg_date']?></dd>
            <input type="button" value="返回列表" class="button" id="return" />
            <input type="button" value="删除申请" class="button" id="delete" name="<?php echo $_GET['id'];?>"/>
        </dl>
    </div>
</div>
<?php
