...
//求出用户人数
$_num=_num_rows("SELECT tg_id FROM tg_message WHERE tg_touser='{$_COOKIE['username']}'");
...
$_sql="SELECT tg_id,tg_fromuser,tg_content,tg_date,tg_state FROM tg_message WHERE tg_touser='{$_COOKIE['username']}' ORDER BY tg_date DESC LIMIT $_pagenum,$_pagesize";
                    ...
                    foreach($_row as $value1){
                        foreach($value1 as $key=>$value2){
                            ...
                            if(empty($_html['tg_state'])){
                                ...
                                $_html['tg_state']='已读';
                                $_html['content']=_title($_html['tg_content']);
                            }
                    ?>
                    <tr>
                        <td><?php echo $_html['tg_fromuser'];?></td>
                        <td><a href="member_message_detail.php?id=<?php echo $_html['tg_id'];?>" title="<?php echo $_html['tg_content'];?>"><?php echo $_html['content'];?></a></td>
                        <td><?php echo $_html['tg_date'];?></td>
                        <td><?php echo $_html['tg_state']?></td>
