...
if($_GET['action']=='send'){
    ...
    $_clean['number']=_check_gift(_formvalue('number'));
    ...
    if(_affected_rows()==1){
        ...
        _close();
        //跳转
        _alert_close('成功送出'.$_clean['number'].'个礼物！');
    }else{
        //关闭数据库
        _close();
        //跳转
...
<div id="message">
    ...
    <form method="post" action="?action=send">
        <dl>
            <dd>
                To:<input type="text" name="touser" value="<?php echo $_html['touser'];?>" class="text">
                <select name="number">
                    <?php
                        foreach(range(1,100) as $_num){
                            echo '<option value="'.$_num.'">'.$_num.'个礼物</option>';
                        }
                    ?>
                </select>
            </dd>
            <dd><textarea name="content">给你送点礼物~~</textarea></dd>
            <?php if($_system['tg_code']==1){?>
            <dd id="cod">验 证 码 ： <input type="text" name="code" class="codeinput" /><img src="code.php" id="code" /></dd>
            <?php }?>
            <dd><input type="submit" value="送 礼 物" class="sub"/><dd>
        </dl>
    </form>
</div>
</body>
