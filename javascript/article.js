...
window.onload=function(){
    ...
    delete_blog.onclick=function () {
        if(window.confirm('确定要删除该博客吗？')){
            location.href='article.php?action=delete_blog&id='+this.name;
        }
    };
    ...
    for(var i=0;i<delete_comment.length;i++){
        delete_comment[i].onclick=function () {
            if(window.confirm('确定要删除该评论吗？')){
                location.href='article.php?action=delete_comment&id='+this.name;
            }
        };
    }
